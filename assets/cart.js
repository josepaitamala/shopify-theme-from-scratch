...
class SideCart extends HTMLElement {
    ...
    renderSideCartItem(context = {}) {
        ...
        element.innerHTML = element.innerHTML
            .replace(/\$\$ITEM_URL\$\$/g, item.url)
            .replace(/\$\$IMAGE_URL\$\$/g, item.featured_image.url ? item.featured_image.url + '&width=100' : 'https://placehold.co/150x150')
            .replace(/\$\$IMAGE_ALT\$\$/g, item.featured_image.alt)
            .replace(/\$\$ITEM_TITLE\$\$/g, item.product_title)
            .replace(/\$\$ITEM__ORIGINAL_PRICE\$\$/g, Shopify.formatMoney(item.original_line_price))
            .replace(/\$\$ITEM_FINAL_PRICE\$\$/g, Shopify.formatMoney(item.final_line_price))
    ...
    buildCart(cart) {
        ...
        if(cart.items.length === 0) {
            ...
            this.querySelector('#sidebar-actions').classList.remove('active');

            console.log('cart-total', this.querySelector('#cart-total'));
            this.querySelector('#cart-total').innerHTML = '';
        } else {
            this.querySelector('#sidebar-actions').classList.add('active');
